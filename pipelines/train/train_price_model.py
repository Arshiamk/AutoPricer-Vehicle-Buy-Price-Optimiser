...
def train_price_models():
    ...
    features = [
        ...
        "vehicle_age", "mileage", "damage_severity_score", 
        "risk_score"
    ]
    target = "sale_price"
    ...
    q10_model = Pipeline(steps=[
        ("preprocessor", preprocessor),
        ("model", GradientBoostingRegressor(loss="quantile", alpha=0.10, n_estimators=100, random_state=42))
    ])
    ...
    q90_model = Pipeline(steps=[
        ("preprocessor", preprocessor),
        ("model", GradientBoostingRegressor(loss="quantile", alpha=0.90, n_estimators=100, random_state=42))
    ])
